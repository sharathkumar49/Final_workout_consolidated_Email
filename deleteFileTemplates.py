...
for tester in ['Alpar Wonerth', 'Anett Rau', 'Annette Opitz', 'Barbara Kronmueller', 'Chinmay Shah', 'Eric Schnurr', 'Florian Brenner', 'Gunasekaran Loganathan', 'Karthick Jayachandran', 'Nagarampalli Kalyan', 'Pavan Palreddygari', 'Robin Mai', 'Sharath Muthu', 'Simone Klasmeier', 'Thanusha Koparthy', 'Unni Rajanarayanan', 'Vijayalakshmi R']:
    ...
    if os.path.exists("./templates/{}_section1_subRed.html".format(str(tester).replace(' ','_'))):
        os.remove("./templates/{}_section1_subRed.html".format(str(tester).replace(' ','_')))

    if os.path.exists("./templates/{}_section1_subNotExecuted.html".format(str(tester).replace(' ','_'))):
        os.remove("./templates/{}_section1_subNotExecuted.html".format(str(tester).replace(' ','_')))
    ...
    if os.path.exists("./templates/exceptiontable.html"):
        os.remove("./templates/exceptiontable.html")
